...
adjust_stacks(C, From, When, IsRootLoop, NewCmd, PosStack, Fun) ->
    ...
    case {NewCmd#cmd.type, When} of
        ...
        {loop, 'after'} when IsRootLoop ->
            ...
            {C2, ActualStop} =
                do_prepare_stop(C, RegExp, <<?loop_break_mismatch>>),
            ...
            receive
                {Port, {exit_status, ExitStatus}}
                  when Port =:= C2#cstate.port ->
                    flush_port(C2, [{shell_exit, ExitStatus}]);
                {'EXIT', Port, PosixCode}
                  when Port =:= C2#cstate.port,
                       C2#cstate.exit_status =/= undefined ->
                    flush_port(C2, [{posix_exit, PosixCode}])
            after 0 ->
                    C2
            end
    end.
...
match_patterns(C) ->
    ...
    match_alt_patterns(C, Actual, Actual, []).
...
match_alt_patterns(C, AltActual, AltRest, LogFuns) ->
    ...
    C2 = match_stop_pattern(C, AltActual, AltRest, LogFuns,
                            fail,
                            <<?fail_pattern_matched>>,
                            C#cstate.fail),

    C3 = match_stop_pattern(C2, AltActual, AltRest, LogFuns,
                            success,
...
match_stop_pattern(C, AltActual, AltRest, LogFuns,
                   StopEvent, StopContext, StopCmd) ->
    {Res, _OptMulti} = match(AltActual, StopCmd),
    case Res of
        {match, Matches} ->
            ExpectPatternMatched = (LogFuns =/= []),
            Actual2 =
                if
                    ExpectPatternMatched ->
                        <<AltActual/binary, AltRest/binary>>;
                    true ->
                        AltActual
                end,
            {C2, ActualStop} =
                prepare_stop(C, Actual2, Matches, StopContext),
        ...
        BreakCmd = #cmd{type = break} ->
            ...
            case Res of
                {match, Matches} ->
                    ...
                    {_Match, Rest} =
                        post_match(C3, AltActual, Matches,
                                   <<?loop_break_matched>>),
...
match_break_patterns(C, _AltActual, AltRest, [], Acc, LogFuns) ->
    %% No break pattern matched. Log previous match and return alternate rest.
    [LF() || LF <- LogFuns],
    C#cstate{actual = AltRest, loop_stack = Acc}.

prepare_stop(C, Actual, Matches, Context) ->
    {Match, Rest} = post_match(C, Actual, Matches, Context),
...
do_flush_port(#cstate{port = Port} = C, VirtualTimeout, Acc0, ExitAcc) ->
    RealTimeout = lux_utils:multiply(VirtualTimeout, C#cstate.multiplier),
    Acc3 =
        case flush_port_data_loop(Port, 0, 0, Acc0) of
            {0, Acc} when VirtualTimeout =/= 0 ->
                {_N, Acc2} = flush_port_data_loop(Port, RealTimeout, 0, Acc),
                Acc2;
            {_N, Acc} ->
                Acc
        end,
    NewData = erlang:iolist_to_binary(lists:reverse(Acc3)),
    C2 = clog_recv(C, NewData),
    Exits = flush_port_exit_loop(C2, RealTimeout, ExitAcc),
    clog_recv_exits(C2, Exits).

flush_port_data_loop(Port, Timeout, N, Acc) ->
    receive
        {Port, {data, Data}} ->
            flush_port_data_loop(Port, Timeout, N+1, [Data | Acc])
    after Timeout ->
            {N,Acc}
    end.
...
clog_recv_exits(C, [Exit | Exits]) ->
    C2 =
        case Exit of
            {shell_exit, ExitStatus} ->
                Events = save_event(C, shell_exit, "status="++?i2l(ExitStatus)),
                C#cstate{state_changed = true,
                         no_more_output = true,
                         exit_status = ExitStatus,
                         events = Events};
            {posix_exit, PosixCode} ->
                Events = save_event(C, posix_exit, "code="++?a2l(PosixCode)),
                C#cstate{events = Events}
        end,
    clog_recv_exits(C2, Exits);
clog_recv_exits(C, []) ->
    C.
...
save_event(#cstate{latest_cmd = _Cmd, events = Events} = C, Op, Data) ->
    case Op of
        recv ->
            ...
            show_debug(C, "recv", Data);
        send ->
            clog(C, Op, "\"~s\"", [lux_utils:to_string(Data)]),
            show_debug(C, "send", Data);
        shell_exit ->
            clog(C, Op, "~s", [lux_utils:to_string(Data)]),
            C;
        posix_exit ->
