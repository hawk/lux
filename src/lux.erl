...
-export([run/4, parse_file/6, interpret_commands/6, annotate_log/2, history/3]).
...
run(File, Opts, PrevLogDir, OrigArgs) ->
    lux_suite:run(File, Opts, PrevLogDir, OrigArgs).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Parse a script file

-spec(parse_file(filename(), run_mode(), boolean(), boolean(), boolean(), opts()) ->
             {ok, filename(), cmds(), opts()} | skip() | error()).

parse_file(File, RunMode, SkipUnstable, SkipSkip, CheckDoc, Opts) ->
    lux_parse:parse_file(File, RunMode, SkipUnstable, SkipSkip, CheckDoc, Opts).
